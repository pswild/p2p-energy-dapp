...
contract Auction1B2P {
  ...
  function finalize() public ended() onlyOwner() {
    ...
    finalized = true;
    beneficiary.transfer(bids[secondPlace]); //only transfer secondPlace's bid
  }

  function refund(address addr) private {
    //require(addr != winner, "winner cannot refund");
    if (addr == winner) {
      uint refundVal = bids[addr] - bids[secondPlace];
      bids[addr] = 0;
      addr.transfer(refundVal);
      //emit Refund(addr, refundVal, now);
    }
